  ...
  describe "split accessor methods" do
    ...
    describe "#starts_at" do
      context 'when nil' do
        ...
        describe 'when modifying #starts_on' do
          ...
          it "returns a date when a valid date string is supplied" do
            date = Date.new(1970, 1, 1)
            model.starts_on = date.strftime("%d/%m/%Y")
            expect(model.starts_at).to eq date
          end
        ...
        context 'when modifying #starts_date' do
          it "returns a date when a valid date is supplied" do
            date = Date.new(1970, 1, 1)
            model.starts_date = date
            expect(model.starts_at).to eq date
          end

          it "returns a date when a valid date string is supplied" do
            date = Date.new(1970, 1, 1)
            model.starts_date = date.strftime("%d/%m/%Y")
            expect(model.starts_at).to eq date
          end
        ...
        context 'when modifying #starts_time' do
          ...
          context 'and a valid #start_date is supplied' do
            ...
            it "returns the date when an invalid time (format) is supplied" do
              model.starts_time = "55:55"
              expect(model.starts_at).to eq model.starts_on
            end

            it "returns the date (no time) when an invalid time (non time) is supplied" do
              model.starts_time = "foo"
              expect(model.starts_at).to eq model.starts_on
            end
          end
      ...
      context 'when set to a datetime' do
        ...
        context 'when modifying #starts_time' do
          ...
          context 'and a valid #start_date is supplied' do
            before { model.starts_date = Date.new(1972, 4, 4) }
    ...
    describe "#starts_on" do
        ...
        context 'when modifying #starts_time' do
          ...
          context 'and a valid #start_date is supplied' do
            ...
            it "returns a date when a valid time string is supplied" do
              model.starts_time = "09:54"
              expect(model.starts_on).to eq Date.new(1970, 1, 1)
            end

            it "returns the date when an invalid time (format) is supplied" do
              model.starts_time = "55:55"
              expect(model.starts_on).to eq model.starts_on
            end
      ...
      context 'when set to a date' do
        before { model.starts_on = Date.new(1975, 3, 4) }
        ...
        context 'when modifying #starts_time' do
          it "returns the original date when a valid date (object) is supplied" do
            model.starts_time = Date.new(1983, 9, 30)
            expect(model.starts_on).to eq Date.new(1975, 3, 4)
          end

          context 'and a valid #start_date is supplied' do
            before { model.starts_date = Date.new(1983, 11, 16) }

            it "returns a date when a valid time string is supplied" do
              model.starts_time = "01:45"
              expect(model.starts_on).to eq Date.new(1983, 11, 16)
            end
    ...
    describe "#starts_date" do
      context 'when nil' do
        it "returns nil" do
          expect(model.starts_date).to be_nil
        end
        ...
        context 'when modifying #starts_at' do
          it "returns a date string when a valid date (object) is supplied" do
            ...
            expect(model.starts_date).to eq date.strftime("%d/%m/%Y")
          end
        ...
        context 'when modifying #starts_on' do
          it "returns a date string when a valid date (object) is supplied" do
            date = Date.new(1982, 6, 7)
            model.starts_on = date
            expect(model.starts_date).to eq date.strftime("%d/%m/%Y")
        ...
        context 'when modifying #starts_date' do
          ...
          it "returns the supplied string when an invalid date (non date) is supplied" do
            invalid_input = "foo"
            model.starts_date = invalid_input
            expect(model.starts_date).to eq invalid_input
          end

          it "returns an empty string when an empty string is supplied" do
            empty_input = ""
            model.starts_date = empty_input
            expect(model.starts_date).to eq empty_input
          end
        end

        context 'when modifying #starts_time' do
          it "returns nil" do
            model.starts_time = "15:53"
            expect(model.starts_date).to be_nil
          end
        end
      end
      ...
      context 'when set to a value' do
        before { model.starts_date = "3/4/2010" }
        ...
        context 'when modifying #starts_on' do
          it "returns the original value when a valid date (object) is supplied" do
            model.starts_on = Date.new(1982, 6, 7)
            expect(model.starts_date).to eq "3/4/2010"
          end
    ...
    describe "#starts_time" do
      context 'when nil' do
        ...
        context 'when modifying #starts_on' do
          it "returns the time string of the date supplied when a valid date (object) is supplied" do
            model.starts_on = Date.new(1993, 6, 7)
            expect(model.starts_time).to eq "00:00"
          end
        ...
        context 'when modifying #starts_date' do
          it "returns the time string of the date supplied when a valid date (object) is supplied" do
            model.starts_date = Date.new(1970, 1, 1)
            expect(model.starts_time).to eq "00:00"
          end
        ...
        context 'when modifying #starts_time' do
          it "returns nil when a valid date (object) is supplied" do
            model.starts_time = Date.new(1970, 1, 1)
            expect(model.starts_time).to be_nil
          end
          ...
          it "returns an empty string when an empty string is supplied" do
            empty_input = ""
            model.starts_time = empty_input
            expect(model.starts_time).to eq empty_input
          end

          it "returns a time string when a valid time string is supplied" do
            time = "01:45"
            model.starts_time = time
            expect(model.starts_time).to eq time
          end

          it "returns the time supplied when an invalid time (format) is supplied" do
            invalid_time = "55:55"
            model.starts_time = invalid_time
            expect(model.starts_time).to eq invalid_time
          end
        end
      end

      context 'when set to a value' do
        before { model.starts_time = "05:05" }
